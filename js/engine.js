...
const Engine = {
  ...
  advanceWeeklyChallenge(challengeId) {
    ...
    if (prog.steps >= challenge.steps) {
      ...
      Render.toast(`🏆 Challenge "${challenge.name}" abgeschlossen! ${LOOTBOX_DEFS[lootType]?.icon} Lootbox!`, 'ach');
      addNotification(`🏆 Weekly Challenge abgeschlossen: <strong>${challenge.name}</strong>`);
      Achievements.checkAll();
    }
